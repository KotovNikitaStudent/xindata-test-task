...
```bash
python main.py --ask "Какие есть категории фрилансеров и сколько их в каждой категории?"
```

### Расширенный режим (SQL + результат + объяснение):

```bash
python main.py --ask "Какие есть категории фрилансеров и сколько их в каждой категории?" --verbose
```
...
| Флаг | Описание |
|------|----------|
| `--is_load_table` | Загружает CSV-файл в SQLite-базу |
...
Система была протестирована на ряде пользовательских запросов к таблице freelancers. Были отобраны как вопросы из задания, так и дополнительные.
...
- Ответ верно интерпретирован LLM (у учетом ошибок выполенения запроса и отсутствия релевантных данных для данного пользовательского вопроса) — в 10/10 случаев.
...
5. Гибкость и расширяемость
  ...
  - Можно заменить датасет, просто загрузив его через --is_load_table.
...
- Избежать необходимости загружать весь датасет в LLM, что запрещено в условиях.
...
- Некорректные вопросы, или вопросы, результатом выполнения которого являются пустые данные. Решено путем переформулированием вопроса, или так же добавлена раняя остановка выполенения запроса в зависимости от 
этапа выполнения, когда была обнаружена ошибка.
